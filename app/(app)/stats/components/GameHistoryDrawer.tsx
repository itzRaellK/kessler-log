  ...
  return (
    <div className={cx(GLASS_CARD, SOFT_RING, "p-4")}>
      <div className="flex items-center justify-between gap-3">
        ...
        {Icon ? <Icon size={16} className="text-emerald-300/90" /> : null}
      </div>
      <div className="mt-2 text-lg font-bold texttracking-tight text-foreground">
        {props.value}
      </div>
    </div>
  );
...
function MiniInfo(props: {
  k: string;
  v: React.ReactNode;
  accent?: "emerald" | "violet" | "sky" | "none";
}) {
  ...
  return (
    <div className={cx("rounded-xl border px-3 py-2", accentCls)}>
      <div className="text-[11px] text-muted-foreground">{props.k}</div>
...
export function GameHistoryDrawer(props: {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <>
          {/* ✅ Backdrop com z-index ALTÍSSIMO */}
          <motion.div
            className="fixed inset-0 z-[2147483640] bg-background/60 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* ✅ Drawer com z-index acima de qualquer header fixo */}
          <motion.aside
            className="fixed right-0 top-0 z-[2147483641] h-full w-full max-w-[980px] overflow-hidden border-l border-border/50 bg-background/70 shadow-2xl backdrop-blur-xl"
          ...
          >
            <div className="flex h-full flex-col">
              ...
              <div className="flex items-center justify-between gap-3 border-b border-border/50 px-5 py-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => onOpenChange(false)}
                  >
                    <X size={16} className="mr-2 text-emerald-300" />
                    Fechar
                  </Button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-auto p-5">
                ...
                {/* ✅ TOP: imagem (esquerda) + infos (direita) */}
                <div className="grid gap-4 grid-cols-1 lg:grid-cols-12">
                  {/* imagem: SEM CORTAR (contain) */}
                  <div className={cx(GLASS_CARD, SOFT_RING, "lg:col-span-5")}>
                    <div className="h-[260px] sm:h-[340px] lg:h-[420px] w-full overflow-hidden rounded-2xl">
                      {overview?.cover_url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={overview.cover_url}
                          alt={overview.title ?? "cover"}
                          className="h-full w-full object-contain bg-background/30 p-3"
                          loading="lazy"
                        />
                      ) : (
                        <div className="flex h-full w-full items-center justify-center bg-background/30 text-muted-foreground">
                          <BookText size={22} />
                        </div>
                      )}
                    </div>
                  ...
                  <div className="lg:col-span-7 space-y-4">
                    <div className={cx(GLASS_CARD, SOFT_RING, "p-5")}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {overview?.external_url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-[11px] font-semibold text-emerald-200 hover:bg-emerald-500/15"
                          >
                      ...
                      {/* ✅ Notas externas: aqui sim emerald */}
                      <div className="mt-4">
                              ...
                              >
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <div className="inline-flex items-center gap-1 text-xs font-bold text-foreground">
                                    <Star
                                      size={12}
                                      className="opacity-90 text-emerald-300"
                                    />
                    ...
                    {/* ✅ linha (grid 3): total ciclos, total sessões, tempo total */}
                    <div className="grid gap-3 grid-cols-1 sm:grid-cols-3">
                      <StatCard
                        label="Total de ciclos"
                        value={overview ? overview.cycles_count : "—"}
                        icon={Layers3}
                      />
                      <StatCard
                        label="Total de sessões"
                        value={overview ? overview.sessions_finished : "—"}
                        icon={Star}
                      />
                      <StatCard
                        label="Tempo total"
                ...
                {/* ✅ Ciclo selecionado: grid 4 */}
                <div className={cx(GLASS_CARD, SOFT_RING, "mt-4 p-5")}>
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    {selectedCycle?.last_session_started_at ? (
                      <div className="text-[11px] text-muted-foreground">
                        última sessão:{" "}
                        {timeAgoShort(selectedCycle.last_session_started_at)}
                      </div>
                    ) : null}
                  </div>
                        ...
                        <MiniInfo
                          ...
                          v={
                            <span className="inline-flex items-center gap-1">
                              <Timer
                                size={14}
                                className="opacity-85 text-emerald-300"
                              />
                        ...
                        <div className="mb-3 grid gap-2 sm:grid-cols-2">
                          ...
                          <label className="flex items-center justify-between rounded-xl border border-border/50 bg-card/40 px-3 py-2">
                            ...
                            <button
                              ...
                              className={cx(
                                SWITCH,
                                onlyWithReview && "bg-emerald-500/20"
                              )}
                              type="button"
                            >
                              <span
                                className={cx(
                                  SWITCH_KNOB,
                                  onlyWithReview ? "left-[18px]" : "left-[2px]"
                                )}
                              />
                            </button>
                          </label>
